...
router.get('/favoritos/favoritos', login.verifyToken, (req, res, next) => {
    ...
    mysql.getConnection((error, conn) => {
        ...
        conn.query(
            'SELECT * FROM tabelaEscritorio WHERE favorito = "Sim";',
            (error, result, fields) => {
                conn.release();
                if (error) { return res.status(500).send({ error: error }) }
...
router.get('/:id', login.verifyToken, (req, res, next) => {
    mysql.getConnection((error, conn) => {
        ...
        conn.query(
            'SELECT * FROM tabelaEscritorio WHERE id = ?;',
            [req.params.id],
            (error, result, fields) => {
                conn.release();
                if (error) { return res.status(500).send({ error: error }) }
...
router.get('/segmento/:vagaPretendida', login.verifyToken, (req, res, next) => {
    ...
    mysql.getConnection((error, conn) => {
        ...
        conn.query(
            'SELECT * FROM tabelaEscritorio WHERE vagaPretendida = ?;',
...
router.get('/pesquisar/pesquisar', login.verifyToken, (req, res, next) => {
    ...
    let query = 'SELECT * FROM tabelaEscritorio';
...
router.patch('/favorito/:id', login.verifyToken, (req, res, next) => {
    ...
    mysql.getConnection((error, conn) => {
        ...
        conn.query(
            `UPDATE tabelaEscritorio SET favorito = ? WHERE id = ?`,
            [favorito, id],
            (error, result, fields) => {
                conn.release();
...
router.delete('/deletar', login.verifyToken, (req, res, next) => {
    ...
    mysql.getConnection((error, conn) => {
      ...
      ids.forEach(id => {
        conn.query(
          'DELETE FROM tabelaEscritorio WHERE id = ?',
          [id],
          (error, result, fields) => {
            if (error) {
              console.log(error);
